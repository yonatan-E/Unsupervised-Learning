...
def plot_model(model, X, points, axe):
    ...
    axe.set_yticklabels([])
    axe.set_xticklabels([])
    axe.scatter(points[:, 0][labels == -1], points[:, 1][labels == -1], label='Outlier', color='m', s=10, alpha=.4)
    axe.scatter(points[:, 0][labels != -1], points[:, 1][labels != -1], label='Standard', color='c', s=10, alpha=.4)
...
if __name__ == '__main__':
    if dataset == 'census':
        ...
        embedder = FAMD(n_components=2, n_iter=10)
        points = embedder.fit_transform(df).values
